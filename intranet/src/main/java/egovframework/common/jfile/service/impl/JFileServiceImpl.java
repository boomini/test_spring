...
public class JFileServiceImpl implements JFileService {
	...
	public void upload(Collection<MultipartFile> multipartFiles, JFileVO fileVo) {		
		...
		for (final MultipartFile file : multipartFiles) {
			...
			if(orgFileName.equals("") || orgFileName ==null || isExceedFileExt(file)) continue;
			upload(file, fileVo);
        }
	}
	...
	/**
	 * 파일삭제 - JFile 밖에서 사용될때 사용
	 * @param fileId
	 */
	public void executeFileDelete(String fileId) {
		updateAttachFileDeleteYnByFileId(fileId, "Y");
		Object[] fileSeqs = getAttacheFileSeqs(fileId);
		if(fileSeqs == null || fileSeqs.length == 0)
			return;
	 ...
	 * @param fileId
	 */
	public void executeFileDelete(String fileId, Object fileSeq) {
	...
	public Integer getMaxFileSeq(String fileId) {
		return mapper.getMaxFileSeq(fileId);
	}
}
