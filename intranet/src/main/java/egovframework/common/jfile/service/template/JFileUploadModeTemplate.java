...
//		jfile.setUseSecurity(fileVo.getUseSecurity());
	...
	public void deleteJFiles(JFileDetails details, String uploadPath, String uploadMode) {
		...
		if(details.getFileThumbnailYn().equals("Y")){
			deleteFileByFullPath(getFileUploadDirectoryPath(uploadMode, uploadPath)+details.getFileThumbnailMask());
		}
	}
	...
	public void upload(MultipartFile multipartFile, JFileDetails fileVo) {
		...
		try
		{
			...
			if (fileVo.getFileThumbnailYn().equals("Y") && fileVo.isImage())
			{
				...
				int width = fileVo.getFileThumbnailWidth();
				int height = fileVo.getFileThumbnailHeigth();
				//이미지 사이즈 조정
				if(bi != null){//웹 취약점 조치
