...
/*
 * 사용방법
 * 페이지 시작시 autoTest(document.formNm); 호출
 *  
 * 클래스 사용
 * autoText   -> Text , val("title+현재시간")
 * autoInt    -> Int  , maxLength 4글자 이상만 가능 , val("현재분+classIndeX")
 * autoSelect -> 1번쨰 option 선택
 * 
 * title , maxlength 필요
 * 
 * */
function autoTest(frm){
	
	if(!confirm("Test 자동완성을 하시겠습니까?")) return;
	
	/*현재시간 구하기*/
	var Now = new Date();
	var currentDay  = Now.getDate(); // 일
	var currentTime = Now.getHours(); // 시
	var currentMin  = Now.getMinutes(); // 분
	/*현재시간 구하기*/
	
	
	//text
	var obj_Text = $(frm).find(".autoText");
	for(var i = 0; i<obj_Text.length; i++){
			
			if($(obj_Text[i]).prop("disabled") || $(obj_Text[i]).attr("type") == "hidden" || $(obj_Text[i]).css("display") == "none"){
				continue;
			}
			var subText = gfnCheckNull($(obj_Text[i]).attr("title"))+currentDay+""+currentTime+""+currentMin ;
			
			if( gfnCheckNull(obj_Text.attr("maxlength"),9999) <  subText.length ){
				subText = currentDay+""+currentTime+""+currentMin ;
			}
			$(frm).find(".autoText:eq("+i+")").val(subText);
		}

	
	//Integer maxLength 4글자 이상
	var obj_Int = $(frm).find(".autoInt");
	for(var i = 0; i<obj_Int.length; i++){
			if($(obj_Int[i]).prop("disabled") || $(obj_Int[i]).attr("type") == "hidden" || $(obj_Int[i]).css("display") == "none"){
				continue;
			}

			var subInt = currentDay+""+currentTime+""+currentMin ;
			if( gfnCheckNull(obj_Int.attr("maxlength"),9999) < subInt.length ){
				subInt = currentMin+""+i ;
			}
			$(frm).find(".autoInt:eq("+i+")").val(subInt);
		}
	
	
	//selectBox
	var obj_Sel = $(frm).find(".autoSelect");
	for(var i = 0; i<obj_Sel.length; i++){
			if($(obj_Sel[i]).prop("disabled") || $(obj_Sel[i]).attr("type") == "hidden" || $(obj_Sel[i]).css("display") == "none"){
				continue;
			}
			$(frm).find(".autoSelect:eq("+i+")").find("option:eq(1)").prop("selected", true);
		}
